...
release:
	...
	aws s3 cp /tmp/lambda.json s3://convox/release/$(VERSION)/formation-lambda.json --acl public-read
ifeq ($(LATEST),yes)
	aws s3 cp /tmp/kernel.json s3://convox/release/latest/formation.json --acl public-read
	aws s3 cp /tmp/lambda.json s3://convox/release/latest/formation-lambda.json --acl public-read
