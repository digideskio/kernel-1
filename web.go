...
func startWeb() {
	...
	router.HandleFunc("/apps/{app}/formation/{process}", api("formation.set", controllers.FormationSet)).Methods("POST")
	...
	router.HandleFunc("/apps/{app}/processes/{process}", api("process.stop", controllers.ProcessStop)).Methods("DELETE")
